...
def log_violent_data_gap_warning(data_logger, failed_dates: List[str], start_date: str, end_date: str):
    ...
    RETRY COMMAND:
    python src/generate_data/collect_data.py --start-date {start_date} --end-date {end_date} --data-dir data
    
    INDIVIDUAL RETRY COMMANDS:
    {chr(10).join([f"    python src/generate_data/collect_data.py --start-date {date} --end-date {date} --data-dir data" for date in failed_dates[:10]])}
