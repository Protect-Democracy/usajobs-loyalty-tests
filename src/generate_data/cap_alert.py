...
the daily workflow turns into a GitHub issue.
...
Ported from abigailhaddad/usajobs_historical (scripts/cap_alert.py). The only
change is the marker path: this repo's collectors live two levels down in
src/generate_data/, not at the repo root, so the marker still resolves to
<repo root>/logs/ regardless of the caller's working directory.
"""
...
def check_cap(count, context):
    ...
    (e.g. "current jobs total" or "current fetch [2210] total").
    """
    try:
        count = int(count)
