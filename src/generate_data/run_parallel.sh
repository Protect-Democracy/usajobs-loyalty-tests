...
for year in "${YEARS[@]}"; do
    ...
    tmux new-session -d -s "$session_name" \
        "cd $(pwd) && source venv/bin/activate && caffeinate -i python $(dirname "$0")/collect_data.py --start-date $start_date --end-date $end_date --data-dir data 2>&1 | tee $logfile"
...
tmux new-session -d -s "usajobs-current-all" \
    "cd $(pwd) && source venv/bin/activate && caffeinate -i python $(dirname "$0")/collect_current_data.py --all --data-dir data 2>&1 | tee $current_logfile"
